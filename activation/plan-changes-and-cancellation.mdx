...
Product scene: a customer starts with the Personal trial, grows to Builder, later removes old devices, then decides whether to keep scaling, cancel, or delete the account.
...
Expected result: a customer should not need to email support for normal plan management, but support and legal links must be easy to find when money, refunds, or deletion are involved.
