...
Product scene: somebody reads the docs, presses `Get My Key`, lands on `account.bitfield.so`, chooses the trial or plan, completes checkout, then opens the account page to activate their first device.
...
<div className="bf-flow" aria-label="Account and key flow">
  ...
  <div className="bf-flow-step">
    ...
    <p>The checkout surface shows trial, plan, active-device range, and cadence.</p>
  </div>
  <div className="bf-flow-arrow">→</div>
  <div className="bf-flow-step">
    <span>3</span>
...
Expected result: a customer should know the account portal is the live place for account access, not the marketing site and not a hand-edited local file.
...
| Choose trial, Personal, Builder, or larger active-device count | Checkout surface |
