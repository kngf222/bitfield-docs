...
This recipe builds a package called `help-package` that carries one text file. The app does not reach into random disk paths. The package declares the file, Runtime Kit checks the path, and the admitted package owns the bytes behind a stable name.
...
| Field | Meaning |
|---|---|
| `type: "stored_bytes"` | This thing stores package-owned bytes. |
...
4. Add a React surface that reads prepared data and asks the target for help.

Keep the same rule: the package declares the bytes; app code uses public Runtime Kit surfaces.

## Next

Read [Callable package slot](/cookbook/callable-package-slot) when the package needs a named target that can do work.

Read [Package boundary](/reference/package-boundary) when you need exact field constraints.
