...
|---|---|
| A product shell with multiple surfaces: launch, help, support, settings, billing, or dashboards. | A single component reading one prepared input. |
| A sidebar/tab/region system that should stay generic. | A one-off hardcoded marketing page. |
| Product areas that need different prepared inputs and targets. | A pure package-boundary example with no UI shell. |
| AI agents keep adding layout special cases. | A reference lookup for one function signature. |

The point is not "make a dashboard." The point is to keep arrangement generic while product meaning lives in package data, targets, descriptors, and surface bodies.

## What you will build

| Layer | File | Public contract |
|---|---|---|
| Package boundary | `launch-product/things-to-store-and-run.json` | Records `welcome-copy`, `checklist`; targets `launch.next-step`, `help.search`. |
| Surface descriptors | `app/surfaces.ts` | Surface ids, labels, regions, component keys, prepared inputs, targets. |
...
|---|---|---|
| `welcome-copy` | Record/prepared input source | `LaunchHomeSurface` |
| `launch-checklist` | Record/prepared input source | `LaunchHomeSurface` |
...
| `label` | Text the shell can render without knowing product meaning. |
...
| `targets` | Public target names this surface may call. |
...
| `launch.support` | none in this fixture | none | public contract placeholder or future body |
...
| Symptom | Cause | Fix |
|---|---|---|
| Shell needs a new `if (surface.id === ...)` branch for every product area. | Product meaning leaked into arrangement. | Move product meaning into descriptors and registry entries. |
...
| Surface reads a prepared input not listed on its descriptor. | The read contract and body drifted. | Add the input to the descriptor or change the surface body. |
| Adding Help requires editing Launch code. | Surface bodies are coupled. | Keep each surface body behind its own `componentKey`. |
| AI agent imports package setup into shell. | Agent treated the shell like product code. | Reject the output; shell only arranges descriptors. |
| Reply parsing crashes. | Target reply contract changed or was never written. | Document request/reply shape and decode only that shape. |

## Safe variations

You can extend this recipe without changing the primitive:

1. Add `launch.analytics` in the `rail` region with its own prepared input.
...
5. Add a support surface that reads account/device status once that public contract exists.

The shell stays generic in every variation. The product grows by adding descriptors, package material, and surface bodies.

## AI review checklist

When an AI builds a product loop from this recipe, require this answer before accepting the code:
...
Read [Package boundary](/reference/package-boundary) for exact package thing fields.
