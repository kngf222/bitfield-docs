...
<p className="bf-lead">
This is the shortest first-result path: get key access, activate a device, add Runtime Kit, read prepared data, and send one target request.
</p>

Product scene: you are building the first screen of a product. It needs one piece of prepared data and one button that asks Bitfield to do work.
...
<div className="bf-flow" aria-label="Quickstart flow">
  ...
  <div className="bf-flow-step">
    ...
    <p>Render prepared data and call a named target.</p>
  </div>
</div>

Expected result: your app can render one prepared value and send one target request without importing private package setup.
...
| A prepared input name and target name | App code reads named prepared data and sends requests to named targets. |
...
Visible result: the screen shows an inbox count when the prepared data is available. The component owns loading, error, and success rendering. It does not decide how Bitfield stores or prepares the data.
...
Visible result: the app asks the named target to do work and waits for a reply. The button does not import the target implementation.
...
| The component stays loading | Device/account path or prepared input is not ready | Check account activation, then read [Runtime Kit troubleshooting](/runtime-kit/troubleshooting) |
| The read returns no data | The prepared input name does not exist yet | Follow [Package to screen](/runtime-kit/package-to-screen) |
| The target request fails | The target name or payload contract is wrong | Read [Send a request](/runtime-kit/send-request) |
| Generated code imports a deep Runtime Kit file | It crossed the public boundary | Use only the imports shown on this page |
