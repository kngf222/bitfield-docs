...
description: "Give an AI agent the Runtime Kit public names so it adds features without importing private implementation code."
---
...
An AI agent needs rails. Without rails, it will do the fastest-looking thing: import unpublished code, parse files in UI code, wire features directly together, or create a second data path. That can work for one screen and punish you later.
...
That is the whole point. An AI agent can move fast without turning your product into a knot.
...
Give an AI agent one of these jobs at a time:
...
The better request names the screen, data name, action name, and files an AI agent may touch.
...
This works because the component reads named data. It does not parse package source.
...
This works because app code uses the public hook. Setup machinery stays behind Runtime Kit.
...
This works because Runtime Kit reads have loading, error, empty, and success states.
...
This works because JSON parsing is valid only when the action's reply shape says the reply is JSON.
...
This works because local state is support evidence. It is not the feature authoring surface.
...
If any row fails, reject the change and make an AI agent repair that exact file, name, or action first.

## Full Runtime Kit review map

Make an AI agent fill this out before you accept the work:
...
If any row is blank, an AI agent has not described the concrete public files and names clearly enough.
...
**Letting an AI agent invent a new public API**

If an AI agent adds a new hook, provider, context, or subscription import, stop it. The current public imports are one request function and one React read hook.

**Letting an AI agent move package setup into React**

React renders named data. Package admission belongs behind Runtime Kit.

**Letting an AI agent make action names too implementation-specific**
...
**Letting an AI agent ignore empty and error states**
