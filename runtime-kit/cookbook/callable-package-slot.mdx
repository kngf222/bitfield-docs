...
<p className="bf-lead">
Use a `slot` when a package needs a named runnable door. App code calls the public action and stays out of how the thing behind that door is built, stored, mounted, or replaced.
</p>
...
A slot is a callable door. The package file says the door name and the methods it accepts. The implementation behind that door stays behind the plug.
...
The action name can stay stable while package-owned bytes change behind it.
```
...
Do not add a second direct app-to-implementation path. Keep the action name as the public door.
