...
<p className="bf-lead">
Use a `slot` when a package needs a named runnable door. App code calls the public target and stays out of how the thing behind that door is built, stored, mounted, or replaced.
</p>

This recipe builds `product.search`, a package-owned target for a search box. The app sends a query. Runtime Kit sends bytes to the target. The target returns reply bytes. The app decodes those bytes according to the public reply contract.
...
| Work that should be called by a stable public target name. | A secret account operation that belongs in account infrastructure. |
...
| `artifact.source_path` | Package-local bytes for the callable thing. |
...
| Reply | Bytes decoded according to the package target's public reply contract. |
...
The target name can stay stable while package-owned bytes change behind it.
```
...
| Target name | The slot name is non-empty and stable. |
...
| Reply contract | The caller decodes bytes only according to the promised reply shape. |

## Common failures

| Symptom | Cause | Fix |
|---|---|---|
| Button throws before work starts. | The target name is empty or not declared. | Use the declared public target name. |
...
| UI cannot parse the reply. | The target returned bytes with a different meaning. | Document and honor the reply contract. |
...
Do not add a second direct app-to-implementation path. Keep the target name as the public door.
...
Read [Package boundary](/reference/package-boundary) for exact `slot` field rules, method-list rules, artifact path rules, and invalid examples.
