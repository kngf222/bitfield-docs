...
<p className="bf-lead">
Build the app-facing side of a Runtime Kit feature: one React surface reads prepared data, renders every state, and asks a named target to do work from a button.
</p>
...
|---|---|
| A welcome panel that reads copy and asks for a refresh. | A package boundary with no UI yet. |
...
| `'welcome'` | Public prepared input name for this surface. |
...
The selector is not a storage path. The component does not know which package record, file, or derived value produced the prepared input.
...
| Prepared input | Reads `welcome` through `useBitfieldData(...)`. |
...
The surface can render real prepared data.
...
| Button works only in local dev | Component imported a target implementation directly. | Call the public target name. |
| Data shape crashes render | Component assumed fields that the prepared input does not provide. | Align `WelcomeData` with the public prepared input contract. |
| AI-generated code creates storage addresses | Agent treated the hook like a database query. | Reject the output and point it to the prepared input contract. |

## AI review checklist

When an AI edits this surface, make it answer:
