...
Product scene: your product grows from one launch surface into launch, help, support, settings, and billing. The shell should still behave like a generic place to put surfaces, not like a product-specific switchboard.
...
A descriptor should be boring data. It gives the shell placement facts and gives the surface body its allowed public handles.
...
| `targets` | Public target names the body may call. | Letting the body import target code. |
...
| A surface reads a package address directly. | Prepared-read boundary was bypassed. | Give the surface a prepared input name and read it with `useBitfieldData(...)`. |
| A button imports the work implementation. | Target boundary was bypassed. | Declare/call a target name with `sendRequestToBitfieldTarget(...)`. |
...
| AI adds a new sidebar branch for every feature. | The primitive was not visible enough. | Point the agent to [Runtime Kit concept map](/runtime-kit/concept-map) and the Cookbook recipe. |

## Build it

The complete Runtime Kit Cookbook recipe lives in [Placeable surface product loop](/runtime-kit/cookbook/placeable-surface-product-loop). It walks through the package boundary, descriptors, shell, surface body, and review checklist as source-owned examples.

Read [Runtime Kit concept map](/runtime-kit/concept-map) when you want to see how placeable surfaces fit with package boundaries, prepared reads, callable targets, and local state.
