...
That code feels fast because everything is reachable. It is also the start of the coupling problem. The editor now knows file tree shape. The AI context knows editor timing. The command palette knows where selection came from. Every future package has to learn the old package graph before it can do a simple job.
