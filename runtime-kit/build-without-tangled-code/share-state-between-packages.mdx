...
A file tree chooses a file, then the editor, breadcrumbs, command palette, and AI context panel all need the active file. That state is no longer a private file-tree detail. It is product coordination state.
...
Traditional code turns the first UI into the source every other package imports:
