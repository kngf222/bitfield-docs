...
Better answer when editor, AI panel, breadcrumbs, and command palette need it:
...
|---|---|---|
| Does another package need this value? | Consider Bitfield state or prepared input. | Keep it private. |
| Does changing it run real product work? | Send a target request. | Keep it private or write product state through the right boundary. |
| Is it only animation, hover, focus, drag, scroll, or draft typing? | Keep it private. | Continue classifying. |
| Would losing it on refresh break product meaning? | It may be product state. | It is probably UI state. |
...
| Does another package need it to make product decisions? | Promote it to a shared fact or prepared input. |
| Does changing it run work? | Use a target request. |
| Is React context being used as product coordination? | Replace it with Runtime Kit reads and requests. |

## Full before and after

Traditional feature request: "When the user clicks a file row, highlight it, open it in the editor, and show the AI panel context."
...
Then the click path updates the public selected-file fact through the package's public boundary. The editor and AI panel read that fact. The hover remains private.
