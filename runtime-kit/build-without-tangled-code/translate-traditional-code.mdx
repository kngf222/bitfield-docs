...
Product scene: you are adding a file tree, editor, preview button, command palette action, sidebar item, or dashboard panel. The code works inside one app today, but it does it by teaching one package the private shape of another package. That is the moment to stop and translate.

Expected result: you can point at each line of traditional code and say which Bitfield path replaces it: shared product fact, prepared input, target request, private UI state, or package boundary.
...
Product job: a note editor lets the user highlight text. Another assistant request area should use that highlighted text. The editor also needs a small debounce so it does not publish every tiny mouse movement.

Traditional mistake: the editor imports the assistant panel store and mutates it directly.
...
Review check: no editor code imports the assistant-request package store.
...
assistant context from selection -> target request or prepared input
...
- Review generated-code rules: [Rules for AI agents](/runtime-kit/build-without-tangled-code/rules-for-ai-agents)
