---
title: "Rules for coding tools"
description: "Tell AI coding tools how to build Bitfield features without falling back to traditional coupled app architecture."
---

<div className="bf-article">

<p className="bf-lead">
AI coding tools will default to traditional app architecture unless the docs teach the Bitfield translation directly. Give the tool a concrete job, the public names it may use, and the private files it must not import or parse.
</p>

Product scene: you ask an AI coding tool to add a feature. It creates a store, imports another package, parses a JSON file, or calls an implementation directly. The code may compile, but it trains the app back into the old architecture.

Expected result: the tool chooses the Bitfield path before writing code.
...
Before writing code, the tool must fill this out:
...
If the tool cannot fill this table out, it should not edit yet.
...
Traditional app shape: the tool reaches for a shared store, direct import, package file read, or framework coordinator because that is what most public examples show.
...
|---|---|
| Did the tool create a shared app store? | It should not. Use Bitfield state for shared product facts. |
| Did the tool import another package's private code? | It should not. Use data names or action requests. |
| Did the tool put visual-only UI state in Bitfield? | It should not. Keep visual-only state private. |
| Did the tool parse `things-to-store-and-run.json` from app code? | It should not. Runtime Kit handles package files. |
| Did the tool call implementation code directly? | It should not. Send an action request. |
| Did the tool make React the architecture? | It should not. React is one adapter around Bitfield reads/requests. |
| Did the tool name data and action requests clearly? | It should. Names should describe the real product job. |

## Common failures

| Tool phrase | What it probably means | Correction |
|---|---|---|
...
At the end of generated work, the tool should report this exact shape:
...
If the report says "none" for everything but the code adds a feature that coordinates multiple packages, the tool probably hid traditional coupling in the implementation. Review the imports and state placement before accepting it.
...
- Use the larger coding-tool page: [Build with coding tools](/runtime-kit/ai-build-rules)
