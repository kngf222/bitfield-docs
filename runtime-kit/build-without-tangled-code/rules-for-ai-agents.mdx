...
| What prepared data is read? | `prepared input: project-preview-surface` or `none` |
| What work is requested? | `target request: selected-agent.update` or `none` |
...
| What public adapter is used? | `React read hook`, `target request`, `native shell read`, or another public adapter |
| What private boundary is avoided? | `no package store imports`, `no package file parsing`, `no direct implementation call` |
...
### Bad: mixed read and work boundary
...
Request work through the target. Read prepared state through the input.
