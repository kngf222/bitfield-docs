...
description: "How package boundaries, prepared reads, callable targets, local state, and placeable surfaces fit together."
---

<div className="bf-article">

<p className="bf-lead">
Runtime Kit keeps product code from tangling by turning package material into two app-facing moves: read prepared data and call named targets.
</p>
...
<div className="bf-flow" aria-label="Runtime Kit concept map from package source to app surface">
  ...
  <div className="bf-flow-node">
    ...
    <p>The app sees prepared input names and target names, not package setup.</p>
  </div>
  <div className="bf-flow-arrow" aria-hidden="true">→</div>
  <div className="bf-flow-node">
...
| Concept | Plain meaning | Public contract |
|---|---|---|
| Package boundary | The package says what it brings. | `things-to-store-and-run.json` with `record`, `stored_bytes`, or `slot` things. |
| Prepared read | The screen asks for data that Runtime Kit already prepared. | `useBitfieldData(inputName)` returns `{ data, loading, error }`. |
| Callable target | The user asks a named door to do work. | `sendRequestToBitfieldTarget({ target, payload }, signal?)` returns reply bytes. |
| Local state | The device has active Bitfield material. | Customer-visible areas under `~/.bitfield`, not hand-edited app data. |
| Placeable surface | A shell places UI by descriptor, not by product branches. | Surface descriptor names region, component key, prepared inputs, and targets. |

## Package boundary

Product scene: you want your welcome copy to ship with a package, not as a hardcoded React string.

A package boundary is the file that says what the package owns. It can declare package-owned records, package-owned bytes, and callable targets.
...
That breaks the shape because the app is parsing package source instead of reading prepared data. The package boundary is for Runtime Kit admission. It is not a UI data API.
...
| Reference | [Package boundary](/reference/package-boundary) |

## Prepared reads

Product scene: the launch screen wants `welcome-copy`, but it should not care which package record, file, or derived value produced it.

A prepared read is a named app-facing value. The component asks for the name and renders the public states.
...
That breaks the shape because the component guessed a storage address. The app should read the prepared input name.
...
Product scene: the user clicks "Suggest next step." The app should call `launch.next-step`, not import the code behind it.
...
That breaks the shape because the button bypasses the target boundary. The target implementation can change only if the app keeps calling the public door.
...
Product scene: one customer says the feature works on a laptop but not a phone. You need to know whether the device has the package, activation, and local Runtime Kit material, without telling the customer to edit files.
...
Product scene: the product grows from one launch screen to launch, help, support, and settings. The shell should not grow a branch for every business area.

A placeable surface descriptor says where a surface can appear and which prepared inputs or targets it may use.
...
That breaks the shape because the shell learns product meaning. The shell should render descriptor labels, regions, and active surface ids.
...
```text
Package declares -> Runtime Kit admits -> app reads prepared inputs or calls targets -> shell places surfaces by descriptor.
```
