...
Runtime Kit keeps the screen from learning package setup.
```
...
The important part is what your app does not do. It does not open storage files. It does not choose data plumbing. It does not parse package files. It does not know how the callable target is backed. That is the point. Your feature code stays calm while the system behind it can get faster, more flexible, and more replaceable.

The fastest way to ruin the shape is to let the first feature cheat. If the first screen imports a package file directly, the second screen will copy that shortcut, the third screen will add a second shortcut, and by month six the product has a maze instead of an architecture.
...
package owns the data and work behind those public names
```
