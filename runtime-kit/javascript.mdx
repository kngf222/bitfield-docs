...
|---|---|---|
| Understand how all Runtime Kit concepts fit together. | [Runtime Kit concept map](/runtime-kit/concept-map) | It connects package boundaries, prepared reads, callable targets, local state, and placeable surfaces. |
...
| Look up exact API shapes. | [Runtime Kit API](/reference/runtime-kit-api) and [Package boundary](/reference/package-boundary) | Reference pages are for exact contracts. |
| Debug a broken feature. | [Troubleshooting](/runtime-kit/troubleshooting) | It maps symptoms to the failed public link. |
| Give generated code rails. | [Build with AI agents](/runtime-kit/ai-build-rules) | It tells code generators what they may edit and what they must not cross. |
...
|---|---|---|
| Package boundary | What does this feature bring? | `things-to-store-and-run.json` declares records, stored bytes, or slots. |
| Prepared read | What does the screen need to show? | `useBitfieldData(...)` reads a prepared input name. |
...
```text
surface asks for prepared data
surface sends a named target request
package owns the data and work behind that public boundary
```
...
| Package | A unit that brings declared data, bytes, or callable doors. | Letting app code import package implementation files. |
...
| Target | A public door app code can ask for work. | Importing the implementation behind the door. |
| Reply bytes | The bytes returned by a target call. | Parsing bytes without a reply contract. |
| Placeable surface | A UI contract a shell can arrange without knowing product meaning. | Hardcoding product-specific layout policy into the shell. |
...
| Give generated code safe rails | [Build with AI agents](/runtime-kit/ai-build-rules) |
...
That tiny contract prevents three common failures: app code reading package files, buttons importing target implementations, and generated code inventing a second data path.
