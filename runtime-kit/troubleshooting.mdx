...
Runtime Kit deliberately hides private machinery from app code. That is good for product architecture, but it means debugging should follow the public chain instead of poking random internals.
...
- **Self-checking feature templates:** Add a review checklist beside every AI-generated Runtime Kit feature.
- **Support-safe diagnostics:** Report package set, input name, target name, and error text without exposing local state contents.
- **Contract-first targets:** Write payload and reply contracts before the UI button calls the target.
- **Boundary-led debugging:** Fix the broken public link instead of creating a second private path.
