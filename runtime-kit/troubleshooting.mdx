...
description: "Find which Runtime Kit boundary broke: package admission, named data reads, target requests, replies, local state, or activation."
---
...
Code tends to connect things directly when the boundary is not repeated clearly. Use [Build with coding tools](/runtime-kit/ai-build-rules) only when the feature is being generated by an AI coding tool.
