...
For exact top-level fields, thing-type fields, valid examples, invalid examples, and path-safety rules, read [Package boundary](/reference/package-boundary). For complete Runtime Kit recipes, read [Runtime Kit Cookbook](/runtime-kit/cookbook/index).
...
That package gives the app three public handles: a prepared checklist input, package-owned help bytes, and the `launch.next-step` target. The app uses those handles instead of importing package setup code.
