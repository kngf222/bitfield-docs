...
Product scene: a screen has a button: "Find the next step." The screen should not import the search code, the ranking code, the package setup, or the storage details. It should send a named request and handle the reply.
...
Expected result: a feature can call a named piece of work without taking a dependency on that piece's private code.
...
| Giving every feature a custom connection path | The product becomes hard to change | Use named targets and package boundaries |
...
- Send a target request: [Send a request](/runtime-kit/send-request)
