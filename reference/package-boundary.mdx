...
<p className="bf-lead">
`things-to-store-and-run.json` is the public package boundary file. It tells Runtime Kit what package-owned records, bytes, and callable targets may be admitted.
</p>
...
Runtime Kit reads this boundary file from the package folder. Other package files only matter when the boundary file refers to them through package-local paths such as `source_path`.
...
|---|---|---:|---|---|
| `package` | string | yes | Must name the package whose folder owns this boundary. | Separates package-owned records, bytes, and targets. |
| `things` | array | yes | Each item must be a supported thing type. | Declares records, stored bytes, and slots. |

Unknown top-level fields may be rejected. The public machine-readable schema is [`reference/package-boundary.schema.json`](/reference/package-boundary.schema.json), and Runtime Kit Cookbook examples are checked against it during `docs:check`.
...
| `slot` | `type`, `name`, `methods`, `boundary`, artifact or build data | One callable target door is admitted. | [Callable package slot](/runtime-kit/cookbook/callable-package-slot) |
...
| Field | Type | Required | Constraints | Meaning |
|---|---|---:|---|---|
| `type` | string | yes | Must be `"slot"`. | Selects the callable-slot thing shape. |
| `name` | string | yes | Public target name. | The name app code calls through `sendRequestToBitfieldTarget(...)`. |
...
| `boundary` | object | yes | Must include `call_shape` and matching public methods. | Describes the call boundary. |
| `boundary.call_shape` | string | yes | Public recipes use `"envelope-bytes-in-envelope-bytes-out"`. | States that the target receives bytes and returns bytes. |
| `boundary.methods` | array of method objects | yes | Should match `methods`. | Repeats the callable public methods at the boundary. |
| `artifact.source_path` | string | artifact path choice | Must resolve inside the package folder. | Prebuilt package-owned target bytes. |
...
## Complete valid package boundary
  ...
  "things": [
    {
      ...
      "payload": {
        ...
        "body": "Every feature stays behind a package boundary."
      }
    },
    {
      "type": "stored_bytes",
...
This file is valid because every declared item is package-owned, each path stays inside the package folder, and the callable target has a public name and method boundary.
...
| Stored bytes or slot artifact is rejected. | The path escapes the package folder. | Remove `../` and absolute paths. Package boundaries cannot claim outside files. |
| App code cannot call the target. | Slot `name`, `methods`, and `boundary.methods` do not describe the public callable door clearly. | Use a stable target name and include `{ "name": "query" }` in both method lists when the recipe is query-shaped. |

## Public versus non-public

| Public boundary may say | Public boundary should not say |
|---|---|
| Package name, record address, stored byte name, package-local source path, target name, method names. | Local absolute paths, private runner setup, hidden build machines, package compiler implementation details. |
| Payload shape owned by your package. | Runtime Kit internal storage layout. |
| Target call boundary: bytes in, bytes out. | How the target implementation is wired internally. |

## Related pages

| Need | Page |
|---|---|
| First package boundary | [Package set with one record](/runtime-kit/cookbook/package-set-with-one-record) |
