...
| `@bitfield/runtime-kit/react` | `useBitfieldData(...)` | React surfaces that read prepared data. | Providers, scopes, selector factories, subscriptions, address readers. |
...
|---|---|---:|---|---|---|
| `target` | `string` | yes | Must be a non-empty public target name. | `"help.search"` | Empty or non-string targets fail before the request is sent. |
| `payload` | `unknown` | no | Converted to bytes before dispatch. Use the target contract to decide how to encode meaning. | `{ query: "pricing" }` | Sending a shape the target does not understand returns a target-level failure. |
...
|---|---|---|---|
| `payload` | `Uint8Array` | Opaque reply bytes from the target. | Decode according to the target contract, not according to Runtime Kit guesses. |
...
This is valid because the app calls the public target name, sends a target-owned JSON payload, and decodes the reply according to the target's documented contract.
...
This is invalid because the app imports the target implementation. The public boundary is the target name plus bytes. Direct implementation imports make the app depend on how the target is built.
...
| Request fails after dispatch | Target or transport failure | Render failure state, keep the user action retryable, and log the target name plus safe payload shape. |
| Reply cannot be decoded | Caller and target disagree on reply contract | Fix the target contract or caller decoder together. |
...
This hook reads prepared data for a React surface. A prepared input is a public name that Runtime Kit already resolved behind the surface. It is not a storage path.

### Selector forms

| Selector | Meaning | Example | Use when |
|---|---|---|---|
| omitted | Read the default prepared input for the current surface. | `useBitfieldData<WelcomeCopy>()` | The surface owns one main data input. |
| string | Read a named prepared input. | `useBitfieldData<Checklist>('launch-checklist')` | The surface has multiple named inputs. |
| object with `input` | Read a named prepared input. | `useBitfieldData({ input: 'welcome-copy' })` | You want object form for clarity or future params. |
| object with `input` and `params` | Read a parameterized prepared input. | `useBitfieldData({ input: 'help-results', params: { query } })` | Runtime Kit owns a prepared input that accepts local selection params. |
...
This is valid because the component asks for a prepared input by name and renders every public state: loading, error, empty, and ready.
...
This is invalid because React code imports private machinery and guesses a storage address. The surface should ask for the prepared input name.
...
| Send target names and payload bytes/data. | Call target implementation files directly. |
| Read prepared input names. | Build storage addresses or parse package files inside components. |
| Decode replies according to the target contract. | Assume Runtime Kit understands target-specific JSON fields. |
...
| Package fields | [Package boundary](/reference/package-boundary) |
