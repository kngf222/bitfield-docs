...
Product scene: somebody sees `0.68ns` and wants to compare it to a managed database query over the internet. That is not the same job. One is a warm local read. The other includes network and server work.
...
Expected result: a reader can tell which comparisons are meaningful and which comparisons are category mistakes.
