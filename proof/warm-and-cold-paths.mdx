...
Product scene: someone asks whether a cold disk fetch, local warm read, and network command can be compared as one "database speed" number. They cannot. The path label is part of the claim.
...
Expected result: a reader does not silently convert one benchmark category into another.
