...
<p className="bf-lead">
A good SDK gives app code a tiny surface and keeps Bitfield setup work behind the plug. The public surface should feel boring because the hard work is already packaged away.
</p>
...
Product scene: a team wants the same package-backed feature to work from React today and a different app surface later. The SDK should preserve the same read/request shape instead of making every app learn setup steps.

<div className="bf-flow" aria-label="SDK boundary flow">
  <div className="bf-flow-step">
    <span>App</span>
    <strong>Calls product functions</strong>
    <p>Read prepared data. Send a named request. Render product states.</p>
  </div>
  ...
  <div className="bf-flow-step">
    ...
    <p>Packages, prepared inputs, targets, local state, and active-device access stay behind the plug.</p>
  </div>
</div>
...
The setup surface can do real work. It may connect to Bitfield, load packages, prepare read scopes, subscribe to updates, decode payloads, validate package boundaries, or recover from resets.
...
| SDK exposes raw package material | The SDK is making app code understand package admission | Expose prepared reads and named requests instead |
...
Expected result: the SDK boundary is right when app code can render a surface with one read call and one request call, while setup work remains invisible to the product component.
