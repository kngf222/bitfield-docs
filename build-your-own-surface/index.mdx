...
<div className="bf-flow" aria-label="Build with Bitfield flow">
  ...
  <div className="bf-flow-step">
    ...
    <p>Render prepared reads and send requests through named public targets.</p>
  </div>
  <div className="bf-flow-arrow">→</div>
  <div className="bf-flow-step">
    <span>3</span>
...
This section also does not ask the shell to become smart. The shell should know placement, navigation, account state, and device state. The package surface should own product body meaning. The callable target should own the requested action.
...
Expected result: the same shape scales from a tiny private tool to a product with many features because each feature can keep its package boundary, surface, and target contract clear.
...
| Read raw package material in app code | The app learns storage shape instead of product meaning | Use prepared reads through the public Runtime Kit surface |
