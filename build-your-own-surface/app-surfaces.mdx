...
<p className="bf-lead">
An app surface is the part your user sees. It should read prepared data, render every state, and send named requests. It should not learn package storage details.
</p>

Product scene: a founder adds a `Customers` screen. The screen needs a list, a loading state, an empty state, and one action: archive a customer.
...
|---|---|---|
| Reads | Which prepared input it needs and how to render each state | Storage layout or package admission |
...
Expected result: the screen can render a real product list, survive loading and error states, and send one named product action without knowing how Bitfield stores or runs the feature.
...
The surface should not parse package material. Package admission and prepared reads belong behind the public Runtime Kit contract.
...
| The screen knows package file names | The surface is bypassing prepared reads | Read through `useBitfieldData(...)` |
| Every button sends a vague command | The target contract is not product-shaped | Use a named target like `customers.archive` |
...
Use [Package authoring](/build-your-own-surface/package-authoring) to create the package boundary that can feed this surface. Use [Runtime Kit API](/reference/runtime-kit-api) for the exact public function contracts.
